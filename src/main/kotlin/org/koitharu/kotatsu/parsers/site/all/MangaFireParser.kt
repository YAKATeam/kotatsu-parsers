		...
		val url = "https://$domain/filter".toHttpUrl().newBuilder().apply {
            ...
            when {
                !filter.query.isNullOrEmpty() -> {
                    val encodedQuery = filter.query.splitByWhitespace().joinToString(separator = "+")
                    addQueryParameter("keyword", encodedQuery)

                    // Generate VRF for search query
                    val searchVrf = getVrfToken("https://$domain/", "vrf")
...
//        val vrf = VrfGenerator.generate("chapter@$chapterId")
    ...
    private suspend fun getVrfToken(url: String, value: String): String = context.requestValueAction(this, url, value)
